...
typedef struct WhirlpoolState {
	u8  bitCounter[LENGTHBYTES]; /* global number of hashed bits (256-bit counter) */
	u8  dataBuffer[WBLOCKBYTES];	/* buffer of data to hash */
	int bufferBits;		        /* current number of bits on the buffer */
	int bufferPos;		        /* current (possibly incomplete) byte slot on the buffer */
	u64 hash[DIGESTBYTES/8];    /* the hashing state */
} WhirlpoolState;

void init(struct WhirlpoolState* const state);
void update(const unsigned char* const data, unsigned long bitLength, struct WhirlpoolState* const state);
void finalize(struct WhirlpoolState* const state, unsigned char* const hashResult);
...
public:
   ...
   * @param bitLength Length of input data in bits
   */
  void update(const uint8_t* data, unsigned long bitLength) {
    ::update(data, bitLength, &state);
  }
  
  /**
   * Finalize the hash and get the result
   * @param digest Buffer for the hash result (must be at least DIGESTBYTES)
   */
